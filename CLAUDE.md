...
每個工地一個獨立知識庫 pool(A 工地查不到 B 工地內容),使用者可上傳 PDF/DOCX/XLSX/PPTX/TXT/MD(單檔 50MB),以自然語言問答並附出處(檔名+頁碼+相關度);工地即時狀態由 LLM tool-calling 直查 MongoDB(`server/routes/aiTools.js` 共 14 工具:統計 9 支——進度、出工/在冊/當月累積人數、許可單〔附表單連結〕、預計出工、違規統計、零事故時數、機具狀態、天氣、工地資料;稽核 5 支——出工人員證照稽核、人員檔案、違規記錄〔他站僅回次數〕、證照到期清查、許可單涵蓋。作業類別→證照對照表 `CATEGORY_CERT_RULES` 為草案待帆宣確認;每次工具呼叫寫 `ai_tool_audit` 留痕。規劃見 `2026_AI_Audit_RAG_Plan.md`),口徑與 Dashboard 一致。規格對齊 `2026_AI_Specs_0417.md`。
...
- **csisserver.exe 自 2026-09-24 起以 Bun 1.4.2 編譯**(exe 內嵌的是「執行 build 的那個 bun」的 runtime)。本機全域 bun 仍是 1.3.14,`yarn build` 會編出 1.3.14 版——部署用 exe 要改用 `~/.bun/bin/bun142.exe build index.ts --compile --target=bun-windows-x64 --outfile csisserver`,並以 yarn 管相依(勿跑 `bun142 install`,會另生 bun.lock 並改動套件)。之後換版舊 exe 一律改名 `csisserver-YYYYMMDD.exe`(日期=舊檔上線日,同日加 `b`、`c`…;新 exe 跟現行版 hash 相同就不要換),步驟照全域 skill `deploy-emicro-server`「編譯 exe 型服務」。伺服器上現有的舊備份是改規則前的命名:`csisserver_1.3.14.exe.bak-20260924`(Bun 1.3.14,目前唯一能退回 1.3.14 的那份)、`csisserver.exe.bak-20260809`(1.3.14)、`csisserver.exe.bak-2026-05-14`(無版本資訊)
