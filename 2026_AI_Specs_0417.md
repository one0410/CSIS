...
| 編號 | 驗收項目 | 驗收標準（可觀測、可測試） |
|------|----------|---------------------------|
| **AC-1.1** | 支援格式文件上傳 | 使用者得於專案範圍內上傳支援格式（PDF／DOCX／TXT／JPG／PNG）之文件，單檔大小不超過 50MB；格式不支援或超過上限時須於介面明確提示並拒絕上傳。 |
| **AC-1.2** | 文件解析與索引入庫 | 系統須於上傳後完成文字解析、切塊（chunking）與向量索引建立，完成後該批文件即納入該專案 RAG 檢索範圍；處理過程須可於列表中觀察到「待處理／處理中／完成／失敗」等狀態。 |
| **AC-1.3** | 已入庫文件列表檢視 | 管理介面須提供已入庫文件清單，至少顯示檔名、上傳者、上傳時間、索引狀態與頁數。 |
| **AC-1.4** | 文件新增／替換／刪除權限控管 | 具權限之使用者得於管理介面新增、替換（覆蓋原檔並重新索引）或刪除文件；操作須依專案與角色權限控管，並記錄稽核日誌（操作者、時間、動作、標的）。 |
| **AC-1.5** | 知識庫由使用者單位自主維護 | 使用者單位得獨立完成知識庫內容之增刪替換，無需依賴廠商代為匯入或維護靜態題庫；操作手冊／規格文件須說明自助維運流程。 |
| **AC-2.1** | 掃描檔／圖片型 PDF OCR 文字擷取 | 對掃描檔、圖片型 PDF 或影像附件（JPG／PNG），系統須擷取版面中之可索引文字，無法識別圖形，並保留頁碼或區塊對應關係，供後續引用定位使用。 |
| **AC-2.2** | OCR 結果納入檢索索引 | OCR 擷取之文字須與原生文字同樣納入向量嵌入與全文檢索流程，且可於 RAG 問答中被檢索與引用，不得僅保存而未索引。 |
| **AC-2.3** | 檔案格式與大小限制落實 | 檔案大小、格式與單檔限制須依「三、核心功能模組」所載技術規格執行；違反限制時須於上傳階段即時拒絕並顯示原因。 |
| **AC-2.4** | OCR 失敗或辨識度過低之明確提示 | 匯入失敗時，系統須明確顯示「辨識失敗」或「辨識度不足」狀態。 |
| **AC-3.1** | 每則回答至少一筆可追溯出處 | 每一則 AI 回答須附至少一筆可追溯之出處項目；若回答包含多段論點，應分別對應其各自的出處。 |
| **AC-3.2** | 出處須含文件名稱 | 每筆出處須顯示原始文件之檔名或標題，使用者可藉此對應到文件列表中之該筆文件。 |
| **AC-3.3** | 出處須含相關度或分數 | 每筆出處須附檢索相關度或分數（如 cosine similarity、rerank score），供使用者判斷引用可信度。 |
| **AC-3.4** | 證據不足時明確標示 | 若檢索結果之相關度或命中數不足以支持斷言，系統須明確標示「無足夠文件依據」或等效語句，不得以模型內部知識補答而不告知。 |
| **AC-3.5** | 禁止偽造出處 | 系統不得產生檔名、頁碼或內容不存在於知識庫之出處；驗收測試須包含對抗性樣本（如詢問知識庫外之內容）以檢驗此項。 |
| **AC-4.1** | 從回答操作檢視來源 | 使用者可於回答介面直接點擊或操作出處項目以檢視該來源（開啟原檔、文件預覽）。 |
| **AC-4.2** | 內嵌預覽不可用時之退路 | 若瀏覽器或檔案格式不支援內嵌預覽，至少應提供原檔下載連結，並於介面文字註明對應頁碼／段落位置，使使用者可於本機開啟後自行定位。 |
...
- **驗收範圍**：OCR 以版面**文字**之擷取與入庫為主；**不包含**對照片、工況影像、複雜圖說等之「圖像級語意解析」。詳見「〇.3 OCR 範圍與限制」。
...
```
優先部署於**自有或委外機房之內網環境**（地端／私有雲），使資料不外送公有雲推理服務。
...
```
向量資料庫：Qdrant（本地部署）
嵌入模型：bge-m3 / bge-large-zh-v1.5（中文優化）(建議)
...
- [ ] 向量資料庫建置
- [ ] 基本問答功能
- [ ] 依「〇.1」撰寫並通過 **AC-1.x～AC-4.x** 各子項驗收測試（含可追溯來源與連結檢視）
...
*文件版本：v1.2*
*建立日期：2026-01-01*
*最後更新：2026-04-17（補充〇節驗收要項、模型版本敘述、OCR 限制與地端部署主張）*
